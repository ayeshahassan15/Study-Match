...
function Groups() {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    if (!form.name.trim() || !form.subject.trim()) { showToast("Name and subject are required.", "error"); return; }
    setCreating(true);
    try {
  ...
  return (
    <div>
      ...
      {state.groups.map(group => {
        const isMember = group.members.includes(myId);
        const isCreator = group.createdBy === myId || group.createdBy?.toString() === myId;
        return (
          <div key={group._id} className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", marginLeft: "1rem" }}>
                {!isMember && (
                  <button onClick={() => handleJoin(group._id)} style={{ fontSize: "0.78rem", padding: "0.3rem 0.7rem" }}>Join</button>
                )}
                {isMember && (
