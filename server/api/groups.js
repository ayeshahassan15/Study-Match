...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      const group = new Group({ name, subject, createdBy: user.id, members: [user.id], memberNames: [user.name] });
      await group.save();
    ...
    if (req.method === "PATCH" && id) {
      ...
      if (group.members.includes(user.id)) return res.status(400).json({ message: "Already a member" });
