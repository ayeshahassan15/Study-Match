...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === "POST" && url.includes("send")) {
      ...
      if (toUser.connectionRequests.map(id => id.toString()).includes(user.id)) return res.status(400).json({ message: "Request already sent" });
      if (toUser.connections.map(id => id.toString()).includes(user.id)) return res.status(400).json({ message: "Already connected" });
    ...
    if (req.method === "PATCH" && url.includes("accept")) {
      ...
      if (!me.connectionRequests.map(id => id.toString()).includes(fromUserId)) return res.status(400).json({ message: "No request found" });
