...
module.exports = async (req, res) => {
  ...
  try {
    if (req.method === "POST") {
      ...
      if (!studentUserId || !rating || rating < 1 || rating > 5) return res.status(400).json({ message: "Invalid rating. Must be between 1 and 5." });
      if (studentUserId === user.id) return res.status(400).json({ message: "Cannot rate yourself." });
    ...
    if (req.method === "GET") {
      ...
      if (!targetUser) return res.status(404).json({ message: "User not found" });
      const avg = targetUser.ratings.count > 0 ? (targetUser.ratings.total / targetUser.ratings.count).toFixed(1) : null;
